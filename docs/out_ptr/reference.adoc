...
* otherwise, `U`.
* This is defined as `phd::out_ptr::pointer_of_or` and `phd::out_ptr::pointer_of_or_t`


[[ref.def.pointer_of]]
- Let `POINTER_OF(T)` denote a type that is defined as `POINTER_OF_OR(T, typename std::pointer_traits<T>::element_type*)`
* This is defined as `phd::out_ptr::pointer_of` and `phd::out_ptr::pointer_of_t`


[[ref.syn]]
## Synopsis

The contents of the library are in namespace phd.OutPtr. There are two template functions: `out_ptr` and `inout_ptr`. There are two types that correspond to each of the 2 template functions, `out_ptr_t` and `inout_ptr_t`:
