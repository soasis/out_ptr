...
TEST_CASE("simple_out_ptr/basic", "simple_out_ptr type works with smart pointers and C-style output APIs") {
	SECTION("unique_ptr<void>") {
		...
		ficapi_create(boost::simple_out_ptr(p), ficapi_type::ficapi_type_int);
	...
	SECTION("unique_ptr<int>") {
		...
		ficapi_int_create(boost::simple_out_ptr(p));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	}
	SECTION("shared_ptr<void>") {
		...
		ficapi_create(boost::simple_out_ptr(p, ficapi::deleter<>()), ficapi_type::ficapi_type_int);
		int* rawp = static_cast<int*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	}
	SECTION("shared_ptr<int>") {
		...
		ficapi_int_create(boost::simple_out_ptr(p, ficapi_int_delete));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	}
...
TEST_CASE("simple_out_ptr/stateful", "simple_out_ptr type works with stateful smart pointers") {
	...
	SECTION("shared_ptr<void>, stateful_deleter") {
		...
		ficapi_create(boost::simple_out_ptr(p, ficapi::stateful_deleter{ 0x12345678, ficapi_type::ficapi_type_int }), ficapi_type::ficapi_type_int);
		int* rawp = static_cast<int*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	...
	SECTION("shared_ptr<int>, stateful_int_deleter") {
		...
		ficapi_int_create(boost::simple_out_ptr(p, ficapi::stateful_int_deleter{ 0x12345678 }));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
	...
	SECTION("shared_ptr<void>, stateful_deleter ref") {
		...
		ficapi_create(boost::simple_out_ptr(p, std::ref(del)), ficapi_type::ficapi_type_int);
		int* rawp = static_cast<int*>(p.get());
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
		REQUIRE(del.type() == ficapi_type::ficapi_type_int);
	...
	SECTION("shared_ptr<int>, stateful_int_deleter ref") {
		...
		ficapi_int_create(boost::simple_out_ptr(p, std::ref(del)));
		int* rawp = p.get();
		REQUIRE(rawp != nullptr);
		REQUIRE(*rawp == ficapi_get_dynamic_data());
